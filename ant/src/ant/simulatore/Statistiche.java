...
public class Statistiche {
	...
	public Map<Formica, Integer> raccoltoPerFormica(Set<Cibo> raccolto) {
		...
		Map<Formica, Integer> formica2raccolto = new HashMap<Formica,Integer>();
		for(Cibo c : raccolto) {
			if(formica2raccolto.containsKey(c.getRaccoglitrice())) {
				Integer intero = formica2raccolto.get(c.getRaccoglitrice());
				formica2raccolto.put(c.getRaccoglitrice(), ++intero);
			} else {
				formica2raccolto.put(c.getRaccoglitrice(), 1);
			}
		}
	...
	public Map<Class<?>, Set<Cibo>> raccoltoPerTipoDiFormica(Set<Cibo> raccolto) {
		...
		Map<Class<?>, Set<Cibo>> raccolto2tipo = new HashMap<>();
		for(Cibo c : raccolto) {
			if(raccolto2tipo.containsKey(c.getRaccoglitrice().getClass())) {
				raccolto2tipo.get(c.getRaccoglitrice().getClass()).add(c);
			} else {
				Set<Cibo> cibo = new HashSet<>();
				cibo.add(c);
				raccolto2tipo.put(c.getRaccoglitrice().getClass(), cibo);
			}
		}
		return raccolto2tipo;
	}
	...
	public List<Class<?>> ordinaStrategieDiRaccolta(final Map<Class<?>, Set<Cibo>> c2c) {
		...
		Collections.sort(listaOrdinata, new Comparator<Class<?>>() {

			@Override
			public int compare(Class<?> arg0, Class<?> arg1) {
				return c2c.get(arg0).size()-c2c.get(arg1).size();
			}
